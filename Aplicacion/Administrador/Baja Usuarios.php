...
<?php
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"
        method="POST">
            <label for="id">Ingresa la ID del usuario a borrar </label>
            <input type="text" name="id" id="id" value="<?php echo $id;?>">
            <input class="btn btn-secondary" type="submit" name="buscarid" value="Buscar ID">
           <br><br>
    ...
    if($query->rowCount()<=0){
       ...
       print("<div class='container mt-3'>");
        ...
        print("</tr>");
            ...
            print("</table>");
            print("</hr>");
            print("<input class='btn btn-danger' type='submit' name='borrar' value='Eliminar usuario'>");
            print("</form>");
...
if (isset($_REQUEST['borrar'])){
    $clave=isset($_REQUEST['id']) ? $_REQUEST['id'] : null;
    $query = $con->prepare('delete FROM canineworld.usuarios where id= :id');
    $query->execute(['id' => $id]);
    if (!$query){
        echo "Error".$query->errorInfo();
    }
    echo "<hr/><hr/>Usuario eliminado.";
    //Cerrar conexión
    $query->closeCursor();
    $query = null;
    $con = null;
